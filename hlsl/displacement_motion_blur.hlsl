...
accum_pixel default_ps(displacement_output IN, SCREEN_POSITION_INPUT(screen_coords)) : SV_Target0
{
	...
	float4 world_space_position= mul(clip_space_position, transpose(ps_displacement_screen_to_world));
	...
	float4 previous_screen_position= mul(previous_world_space_position, transpose(ps_displacement_previous_view_projection));
	previous_screen_position/= previous_screen_position.w;
	
	const float2 max_blur= ps_displacement_blur_max_and_scale.xy;
	const float2 motion_blur_scale= ps_displacement_blur_max_and_scale.zw;
	
	float2 crosshair_relative_position= screen_position.xy - ps_displacement_crosshair_center.xy;
	float center_falloff_scale_factor= min(1.0f, dot(crosshair_relative_position.xy, crosshair_relative_position.xy) * ps_displacement_misc_values.w);
	
	float2 screen_position_delta= screen_position.xy - previous_screen_position.xy;
	screen_position_delta*= motion_blur_scale; // I optimized by premultiplying ps_displacement_misc_values.y into the motion blur scale;
	
	float2 frame_texcoords= IN.Texcoord;
	 
	if (ps_displacement_do_distortion)
	{
		...
		float2 displacement= 2.0f * max_displacement * ps_displacement_screen_constants.zw * (sample2D(displacement_sampler, IN.Texcoord).xy - distortion_offset);
	...
	float inverse_num_taps= 1.0f / ps_displacement_misc_values.x;
	...
	{
		for (int i = 0; i < ps_displacement_num_taps; ++ i)
		{
