...
LOCAL_SAMPLER_2D_IN_VIEWPORT_MAYBE(previous_sampler, 1);
...
float4 default_ps(screen_output IN) : SV_Target
{
	float2 sample0= IN.texcoord; // - 0.5 * source_pixel_size;
 	...
 	sample0.x += ps_postprocess_pixel_size.x;
	color += 2*convert_from_bloom_buffer(sample2D(previous_sampler, sample0));
 	sample0.x += ps_postprocess_pixel_size.x;
	color += 2*convert_from_bloom_buffer(sample2D(previous_sampler, sample0));
	
	sample0.x -= ps_postprocess_pixel_size.x*2;
	sample0.y += ps_postprocess_pixel_size.y;
	color += 2*convert_from_bloom_buffer(sample2D(previous_sampler, sample0));
 	sample0.x += ps_postprocess_pixel_size.x;
